...
const content = `// generated from generate_execute.js at ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()} 
...
`
try {
    await writeFile("./gameboy-emu/opcodes/execute.ts", content)
    console.log("wrote execute.ts")
} catch (err) {
    console.error(err)
}
